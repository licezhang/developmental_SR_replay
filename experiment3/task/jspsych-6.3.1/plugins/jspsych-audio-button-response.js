...
jsPsych.plugins["audio-test"] = (function () {
    ...
    parameters: {
      ...
      correct_answer: {
        ...
        pretty_name: 'Correct answer',
				default: undefined,
				description: 'The correct response (numeric)'
			},
      //</new>
      choices: {
      ...
      incorrect_prompt: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Incorrect Prompt',
        default: null,
        description: 'Any content here will be displayed below the stimulus after an incorrect response.'
      },
      //</new>
      trial_duration: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    function after_response(choice) {
      ...
      // var btns = document.querySelectorAll('.jspsych-audio-test-button button');
      ...
      } else if (response.button == trial.correct_answer) {
        if (trial.response_ends_trial) {
              end_trial();
            }
      } else {
        // console.log(response.button)
        // console.log(trial.correct_answer)      
  
      
        if (true) { // commented out, because apparently alerts can kick someone out of fullscreen mode
          // alert(trial.incorrect_prompt);
        } else { // just adds a line with the incorrect prompt 
          display_element.innerHTML += " " + "<strong>" + trial.incorrect_prompt + "</strong>";
        }
