...
```{r learning model by stage}
learning_mod_by_stage <- glmer(optimal ~ age_z * trial_z * stage + (trial_z * stage | subject_id),
                  data= learning_data_by_stage,
                  family = binomial,
                  contrasts = list(stage="contr.sum"))

my_tab_model(learning_mod_by_stage)
```

### Looking at replanning when excluding participants who performed poorly (<75% accurate) on second stage test
...
learning_mod <- glmer(correct ~ age_z * trial_z * block_order * block_condition + ( trial_z * block_condition | subject_id), 
                        data = learning_data,
                        family = binomial,
                  contrasts = list(block_condition="contr.sum"))
my_tab_model(learning_mod)

relearning_mod <- glmer(correct ~ age_z * trial_z * block_condition * block_order + (trial_z * block_condition | subject_id),
                  data= relearning_data,
                  family = binomial,
                  contrasts = list(block_condition="contr.sum"))
my_tab_model(relearning_mod)

reval_mod <- lmer(reval_score ~  age_z * block_condition * block_order + (1 | subject_id),
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
my_tab_model(reval_mod)

second_stage_test_mod <- lmer(second_stage_test ~ age_z * block_order * block_condition + (1 | subject_id),
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
my_tab_model(second_stage_test_mod)
