...
ggplot(data, aes(x=block_condition, y=second_stage_test, fill=block_condition)) +
  geom_bar(stat="summary", fun="mean", position=position_dodge(width=0.7), width=0.8, color="black") +
  geom_point(position=position_jitter(width=0.15, height=0), alpha=0.5, shape=21, color="black") +
  geom_errorbar(
    data = means,
    aes(
      x = block_condition,
      y = second_stage_test,
      ymin = second_stage_test - se,
      ymax = second_stage_test + se
    ),
    width = 0.2,
  ...
  ylab("Second Stage Test Accuracy") +
  theme_classic(base_size = 14) + 
  scale_fill_manual(values=c("#F8746B", "#10BEC3")) +
  theme(axis.text.x=element_text(size=10), legend.position="none", aspect.ratio=6/9)
