...
```{r revaluation 1}
reval_mod <- lm(reval_score ~  age_z * block_condition * second_stage_test,
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
...
subj_means <- summarySE(data, measurevar="second_stage_test", groupvars=c("block_condition","block_order","subject_id"))
means <- summarySE(subj_means, measurevar="second_stage_test", groupvars=c("block_condition","block_order"))
means$block_order <- factor(means$block_order, labels=c("First","Second"))

ggplot(means, aes(x=block_order,y=second_stage_test, fill=block_condition)) +
  geom_bar(stat = "identity",position="dodge") +
  geom_errorbar(aes(ymin = second_stage_test - se, ymax = second_stage_test + se), position = position_dodge(width = 0.9), width = 0.25) 
```
