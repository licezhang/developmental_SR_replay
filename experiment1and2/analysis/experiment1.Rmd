...
learning_data <- read_csv("../data/experiment1_rest/preprocessed/learning_data1.csv",show_col_types = FALSE)
...
learning_data2 <- read_csv("../data/experiment1_rest/preprocessed/learning_data2.csv",show_col_types = FALSE)
...
```{r learning model 1}
learning_mod_1 <- glmer(is_optimal ~ age_z * trial_z * block_condition + (trial_z + block_condition | subject_id), 
                        data = learning_data,
                        family = binomial,
...
ggsave("figures/experiment1_learning.png")
```
...
# last_10_accuracy ~ age_z * block_condition + (block_condition |subject_id) ===> 
...
ggsave("figures/experiment1_relearning.png")
```
...
reval_mod_agesq <- lm(reval_score ~  (age_z + age_squared_z) * block_condition,
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
my_tab_model(reval_mod_agesq)
```
...
second_stage_test_mod <- lm(second_stage_accuracy ~ age_z * block_condition,
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
my_tab_model(second_stage_test_mod)
# second_stage_accuracy ~ age_z * block_condition + (1|subject_id) ===> isSingular
```
...
learning_mod_1 <- glmer(is_optimal ~ age_z * trial_z * block_order * block_condition + (1 | subject_id), 
                        data = learning_data,
                        family = binomial,
                  control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)),
                  contrasts = list(block_condition="contr.sum"))
...
# is_optimal ~ age_z * trial_z * block_order * block_condition + (trial_z + block_order || subject_id ===> isSingular
# is_optimal ~ age_z * trial_z * block_order * block_condition + (trial_z || subject_id ===> isSingular
# is_optimal ~ age_z * trial_z * block_order * block_condition + (1 | subject_id ===> SUCCESS
...
second_stage_test_mod <- lm(second_stage_accuracy ~ age_z * block_order * block_condition,
                   data=data,
                   contrasts = list(block_condition="contr.sum"))
my_tab_model(second_stage_test_mod)
# second_stage_accuracy ~ age_z * block_order * block_condition + (1|subject_id) ===> isSingular
```

```{r block order revaluation interaction effect plot}
subj_means <- summarySE(data, measurevar="reval_score", groupvars=c("block_condition","block_order","categorical_age","subject_id"))
means <- summarySE(subj_means, measurevar="reval_score", groupvars=c("block_condition","block_order","categorical_age"))
means$categorical_age <- factor(means$categorical_age, levels = c('Children', 'Adolescents', 'Adults'))
means$block_order <- factor(means$block_order, labels=c("First","Second"))

ggplot(means, aes(x=block_order,y=reval_score, fill=block_condition)) +
  geom_bar(stat = "identity",position="dodge") +
  geom_errorbar(aes(ymin = reval_score - se, ymax = reval_score + se), position = position_dodge(width = 0.9), width = 0.25) +
  facet_wrap(~ categorical_age, scales = "free") + 
  scale_y_continuous(limits = c(0, 0.6)) 
```

```{r block order interaction 2nd stage test plot}
subj_means <- summarySE(data, measurevar="second_stage_accuracy", groupvars=c("block_condition","block_order","subject_id"))
means <- summarySE(subj_means, measurevar="second_stage_accuracy", groupvars=c("block_condition","block_order"))
means$block_order <- factor(means$block_order, labels=c("First","Second"))

ggplot(means, aes(x=block_order,y=second_stage_accuracy, fill=block_condition)) +
  geom_bar(stat = "identity",position="dodge") +
  geom_errorbar(aes(ymin = second_stage_accuracy - se, ymax = second_stage_accuracy + se), position = position_dodge(width = 0.9), width = 0.25) 
```
...
ggplot(data[data$block_condition == "Revaluation", ], aes(y=reval_single_rt, x=reval_score, color=categorical_age)) +
  geom_point() +
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2"))) +
  #stat_smooth(aes(y=rt0, x=reval_score, color=categorical_age),method='lm', alpha=0.2) +
  ylim(0, 3000)

rt_mod <- lmer(rt0 ~ age_z * reval_score + (1 |subject_id), 
                  data=data,
                  control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)),
                  contrasts = list(block_condition="contr.sum"))
my_tab_model(learning_mod_2)
